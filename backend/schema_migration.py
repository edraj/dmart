...
FIELD_TYPE_PARSER: dict = {"string": str, "number": int, "integer": int, "array": list}


async def change_field_type(space: str, schema_model: Schema, schema_payload: dict, field: str, new_type: FieldType):
    ...
    await db.internal_sys_update_model(
        ...
        updates={"properties": schema_properties},
    )
    ...
    for one in path.glob(entries_glob):
        ...
        resource_cls = getattr(sys.modules["models.core"], helpers.camel_case(resource_type))
        try:
            resource_obj: Meta = await db.load(space_name=space, subpath=subpath, shortname=shortname, class_type=resource_cls)

            # 5-if resource.schema_shortname == schema:
            if (
                not resource_obj.payload
                or not isinstance(resource_obj.payload.body, str)
                or resource_obj.payload.schema_shortname != schema_model.shortname
            ):
                continue

            # 5.1-load payload file
            resource_payload = await db.load_resource_payload(
                space_name=space, subpath=subpath, filename=resource_obj.payload.body, class_type=resource_cls
            )
        ...
        await db.internal_sys_update_model(
            ...
            updates={field_tree[0]: main_field},
        )
        updated_num += 1
...
async def main(space: str, schema: str, field: str, old_type: FieldType, new_type: FieldType):
    """
    Algorithm:
    1-load schema
    ...
    schema_model: Schema = Schema.model_validate(
        await db.load(space_name=space, subpath="schema", shortname=schema, class_type=Schema)
    )
    if (
        not schema_model.payload
        or schema_model.payload.content_type != ContentType.json
        or not isinstance(schema_model.payload.body, str)
    ):
        print(f"Invalid schema file: \n{schema_model.model_dump_json()}")
        return
    schema_payload = await db.load_resource_payload(
        space_name=space, subpath="schema", filename=schema_model.payload.body, class_type=Schema
    )
    ...
    updated_num = await change_field_type(space, schema_model, schema_payload, field, new_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--old-type", help="Field's old type, supported types: string, number, integer, array")
    parser.add_argument("--new-type", help="Field's new type, supported types: string, number, integer, array")

    args = parser.parse_args()

    asyncio.run(main(args.space, args.schema, args.field, args.old_type, args.new_type))
