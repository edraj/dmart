...
async def main(space: str, subpath: str, schema_path: str, num: int):
    ...
    for _ in range(0, num):
        ...
        await db.internal_save_model(space_name=space, subpath=subpath, meta=meta, payload=payload)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--space", help="Store records under this space")
    parser.add_argument("-s", "--subpath", help="Store records under this subpath")
    parser.add_argument("-c", "--schema-path", help="Generate records according to this schema (path)")
    parser.add_argument("-n", "--num", help="Number of records to be generated", type=int)

    args = parser.parse_args()

    asyncio.run(main(args.space, args.subpath, args.schema_path, args.num))
