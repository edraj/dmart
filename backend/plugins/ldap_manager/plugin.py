...
class Plugin(PluginBase):
    def __init__(self) -> None:
        ...
        try:
            ...
            self.conn = Connection(server, user=settings.ldap_admin_dn, password=settings.ldap_pass, auto_bind=AUTO_BIND_NO_TLS)
        except Exception:
            logger.error("Failed to connect to LDAP")
    ...
    async def hook(self, data: Event):
        ...
        if not isinstance(data.shortname, str):
            logger.warning("data.shortname is None and str is required at ldap_manager")
            return
        ...
        user_model: User = await db.load(
            space_name=settings.management_space, subpath=data.subpath, shortname=data.shortname, class_type=User
        )
    ...
    def add(self, shortname: str, user_model: User):
        self.conn.add(
    ...
    def modify(self, shortname: str, user_model: User):
        self.conn.modify(
            f"cn={shortname},{settings.ldap_root_dn}",
            {
                "gn": [(MODIFY_REPLACE, [str(getattr(user_model, "displayname", "")).encode()])],
                "userPassword": [(MODIFY_REPLACE, [getattr(user_model, "password", "").encode()])],
            },
        )
