...
class RedisDB(BaseDB):
    ...
    async def get_count(
        ...
        try:
            async with RedisServices() as redis:
                return await redis.get_count(space_name, schema_shortname)
    ...
    async def prepare_meta_doc(
        self, space_name: str, subpath: str, meta: Meta
    ) -> tuple[str, dict[str, Any]]:
        async with RedisServices() as redis:
            return redis.prepate_meta_doc(space_name, subpath, meta)
        ...
        try:
            async with RedisServices() as redis:
                lock_payload = await redis.get_lock_doc(
                    dto.space_name,  dto.subpath, dto.shortname
                )
                if lock_payload:
