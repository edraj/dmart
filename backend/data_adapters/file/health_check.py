...
def print_header() -> None:
    print("{:<32} {:<6} {:<6}".format("subpath", "valid", "invalid"))
        ...
        while True:
            ...
            for redis_doc_dict in res_data:
                ...
                if not payload_doc_content and redis_doc_dict.get("payload_doc_id"):
                    payload_redis_doc = await redis.get_doc_by_id(redis_doc_dict["payload_doc_id"])
                ...
                if meta:
                    try:
                        ...
                        if (
                            ...
                            and meta.payload.client_checksum
                            and meta.payload.checksum != meta.payload.client_checksum
                        ):
                            raise Exception(f"payload.checksum not equal payload.client_checksum {subpath}/{meta.shortname}")
                ...
                for redis_doc_dict in res_data.docs:
                    ...
                    if isinstance(redis_doc_dict, dict):
                        ...
                        if not key_entries[key].get(value):
                            ...
                            if (
                                not duplicated_entries[key].get(value)
                                or key_entries[key][value] not in duplicated_entries[key][value]["loc"]
                            ):
            ...
            records=[
                core.Record(
                    ...
                    attributes={
                        ...
                        "payload": {"schema_shortname": "health_check", "content_type": ContentType.json, "body": health_check},
                    },
                )
            ],
        ),
                        ...
                        if isinstance(redis_doc_dict, dict):
                            if "uuid" in redis_doc_dict:
                                ...
                                if "uuid" in redis_doc_dict and redis_doc_dict["uuid"] in uuid_scanned_entries:
                                    ...
                                    uuid_duplicated_entries.setdefault(short_uuid, {"loc": [], "total": 0})
                            ...
                            if "slug" in redis_doc_dict and redis_doc_dict["slug"] in slug_scanned_entries:
                                slug_duplicated_entries.setdefault("slug", {"loc": [], "total": 0})
                ...
                core.Record(
                    ...
                    attributes={
                        ...
                        "payload": {
                            ...
                            "body": {"entries": {"uuid": uuid_duplicated_entries, "slug": slug_duplicated_entries}},
                        },
                    },
                )
            ],
...
async def cleanup_spaces() -> None:
    ...
    if not os.path.isfile(Path(settings.spaces_folder / "management/schema/health_check.json")) or not os.path.isfile(
        Path(settings.spaces_folder / "management/schema/.dm/health_check/meta.schema.json")
    ):
        meta = core.Schema(
        ...
        schema = {"type": "object", "title": "health_check", "additionalProperties": True, "properties": {}, "required": []}
