...
@pytest.mark.anyio
async def test_get_settings_should_fail(client: AsyncClient) -> None:
    invalid_token = await sign_jwt({"username": "test_user"}, settings.jwt_access_expires)
    client.cookies.set("auth_token", invalid_token)
    response = await client.get("/info/settings")
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    response_json = response.json()
    assert response_json["error"]["type"] == "access"
    assert response_json["error"]["code"] == InternalErrorCode.NOT_ALLOWED
    assert response_json["error"]["message"] == "You don't have permission to this action"
