...
# Test for redis_doc_to_meta function
def test_redis_doc_to_meta():
    ...
    with patch("models.core.Record") as MockRecord:
        ...
        assert MockRecord.model_fields.keys.call_count == 3  # Adjusting for the multiple calls
...
# Test for main function handling argument parsing
@pytest.mark.asyncio
...
async def test_main(mock_archive, mock_parse_args):
    ...
    with patch("asyncio.run") as mock_asyncio_run:
        mock_asyncio_run.side_effect = lambda x: asyncio.ensure_future(x)  # Ensure the coroutine is awaited
        main()
