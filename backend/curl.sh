...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"space_name":"management","request_type":"create","records":[{"resource_type":"user","subpath":"users","shortname":"distributor","attributes":{"roles": ["distributor_admin"], "msisdn": "7895412658", "email": "dummy_unqiue@gmail.com"}}]}' ${API_URL}/managed/request | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "update user from admin: \t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"space_name":"management","request_type":"update","records":[{"resource_type":"user","subpath":"users","shortname":"distributor","attributes":{"roles": ["ros"], "msisdn": "7895412658", "email": "dummy_unqiue@gmail.com"}}]}' ${API_URL}/managed/request | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Verify Email/msisdn admin side: "
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"space_name":"management","request_type":"update","records":[{"resource_type":"user","subpath":"users","shortname":"distributor","attributes":{"is_email_verified":true,"is_msisdn_verified":true}}]}' ${API_URL}/managed/request | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Reset user from admin side\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d $'{"shortname": "distributor"}' ${API_URL}/user/reset | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Delete user from admin: \t" 
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"space_name":"management","request_type":"delete","records":[{"resource_type":"user","subpath":"users","shortname":"distributor","attributes":{}}]}' ${API_URL}/managed/request | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

#echo -n -e "Health check: \t\t\t"
#curl -s -H "Authorization: Bearer $AUTH_TOKEN" ${API_URL}/managed/health/management | jq .status | tee /dev/stderr | grep "success" > /dev/null
#RESULT+=$?


echo -n -e "Collection Contacts Query: \t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"type": "search","space_name": "ordering","subpath": "collections/contacts","retrieve_json_payload": true,"search": "","retrieve_attachments": true}' ${API_URL}/managed/query | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
...
echo -n -e "JQ filter: \t\t\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"type": "search","space_name": "aftersales","subpath": "/tickets","filter_schema_names": ["rc_compensation","connect_disconnect","add_remove_vas"],"filter_types": ["ticket"],"search": "","jq_filter" : ".[].shortname"}' ${API_URL}/managed/query | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?


echo -n -e "Reload security: \t\t"
curl -s -b mycookies.jar -H "$CT" -H "Authorization: Bearer $AUTH_TOKEN" ${API_URL}/managed/reload-security-data  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$DELETE" ${API_URL}/managed/space  | jq .status | tee /dev/stderr | grep "success" > /dev/null
# RESULT+=$?
...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$CREATE" ${API_URL}/managed/space  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?


echo -n -e "Query spaces: \t\t\t"
RECORD=$(jq -c -n '{space_name: "dummy", type: "spaces", subpath: "/"}')
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$RECORD" ${API_URL}/managed/query  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$REQUEST" ${API_URL}/managed/request | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
SUBPATH="posts"
...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$REQUEST" ${API_URL}/managed/query | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?



echo -n -e "Create Schema for workflows: \t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"'  'request_type: "create"' -F 'request_record=@"../sample/test/createschemawork.json"' -F 'payload_file=@"../sample/test/workflow_schema.json"' ${API_URL}/managed/resource_with_payload | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?


echo -n -e "Create content for workflows: \t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"' -F 'request_record=@"../sample/test/createticket.json"' -F 'payload_file=@"../sample/test/ticket_workflow.json"' ${API_URL}/managed/resource_with_payload  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?


echo -n -e "Create Schema for ticket: \t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"' -F 'request_record=@"../sample/test/createschema.json"' -F 'payload_file=@"../sample/test/schema.json"' ${API_URL}/managed/resource_with_payload | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Create ticket: \t\t\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"'  'request_type: "create"' -F 'request_record=@"../sample/test/ticketcontent.json"' -F 'payload_file=@"../sample/test/ticketbody.json"' ${API_URL}/managed/resource_with_payload  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?


echo -n -e "Create QR Code: \t\t"
TEMP_FILE=$(mktemp)
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" --output $TEMP_FILE  ${API_URL}/qr/generate/ticket/dummy/myfolder/an_example # | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
file -ib $TEMP_FILE 
rm -f $TEMP_FILE 
#curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F  ${API_URL}/qr/generate/ticket/dummy/myfolder/an_example  | jq .status | tee /dev/stderr | grep "success" > /dev/null
...
# curl -s -X "PUT" -H "Authorization: Bearer $AUTH_TOKEN" ${API_URL}/managed/lock/ticket/dummy/myfolder/an_example | jq .status | tee /dev/stderr | grep "success" > /dev/null

# echo -n -e "Unlock profile: \t"
# curl -s -X "DELETE" -H "Authorization: Bearer $AUTH_TOKEN" ${API_URL}/managed/lock/dummy/myfolder/an_example | jq .status | tee /dev/stderr | grep "success" > /dev/null
...
echo -n -e "Create content: \t\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"' -F 'request_record=@"../sample/test/createcontent.json"' -F 'payload_file=@"../sample/test/data.json"' ${API_URL}/managed/resource_with_payload  | jq .status | tee /dev/stderr | grep "success" > /dev/null 
RESULT+=$?
...
curl -s -H "accept: text/csv" -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{"space_name":"dummy","subpath":"myfolder","type":"subpath","retrieve_json_payload":true,"limit":5}' ${API_URL}/managed/csv | xargs -0 echo | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
...
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" --data-binary "@../sample/test/updatecontent.json" ${API_URL}/managed/request  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Delete content: \t\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" --data-binary "@../sample/test/deletecontent.json" ${API_URL}/managed/request  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?

echo -n -e "Upload attachment: \t\t"
curl -s -H "Authorization: Bearer $AUTH_TOKEN" -F 'space_name="dummy"' -F 'request_record=@"../sample/test/createmedia.json"' -F 'payload_file=@"../sample/test/logo.jpeg"' ${API_URL}/managed/resource_with_payload  | jq .status | tee /dev/stderr | grep "success" > /dev/null
RESULT+=$?
...
#curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d "$RECORD" ${API_URL}/managed/reload-redis-data | jq .status | tee /dev/stderr | grep "success" > /dev/null

#echo -n -e "Delete admin: \t\t"
#curl -s -H "Authorization: Bearer $AUTH_TOKEN" -H "$CT" -d '{}' $API_URL/user/delete | jq .status | tee /dev/stderr | grep "success" > /dev/null
