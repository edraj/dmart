    ...
    async def __call__(self, request: Request) -> str | None:  # type: ignore
        ...
        if not auth_token:
            raise api.Exception(
                ...
                api.Error(
                    type="jwtauth",
                    code=InternalErrorCode.NOT_AUTHENTICATED,
...
async def get_redis_active_session(user_shortname: str):
    async with RedisServices() as redis:
        return await redis.get(f"active_session:{user_shortname}")


async def remove_redis_active_session(user_shortname: str) -> bool:
    async with RedisServices() as redis:
        return bool(await redis.del_keys([f"active_session:{user_shortname}"]))
