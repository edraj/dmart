    ...
    for receiver in receivers:
        ...
        if "mobile" in notification_dict["types"]:
            try:
                ...
                # )
                # await send_notification_firebase(
                #     user, notification_dict, entry.get("shortname")
                # )
            except Exception as e:
                logger.error(
                    "Notification",
    ...
    for field, value in entry_dict.items():
        ...
        if field in ["created_at", "updated_at"]:
            message = message.replace(
                f"{{{field}}}", str(str(value).strftime("%Y-%m-%d %H:%M:%S"))
            )
        else:
...
# async def send_notification_firebase(user: User, data, entry_id):
#     users_fcm_tokens = {}
...
#             data={**data.get("deep_link"), "id": entry_id},
#         )
