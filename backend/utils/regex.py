...
SUBPATH = r"^[\w\/]{1,128}$"
SHORTNAME = r"^\w{1,64}$"
FILENAME = re.compile(r"^\w{1,32}\.(gif|png|jpeg|jpg|pdf|wsq|mp3|mp4)$")
SPACENAME = r"^\w{1,32}$"
EXT = r"^(gif|png|jpeg|jpg|json|md|pdf|wsq|mp3|mp4)$"
...
FILE_PATTERN = re.compile("\\.dm\\/(\\w*)\\/meta\\.([a-zA-Z]*)\\.json$")
PAYLOAD_FILE_PATTERN = re.compile("(\\w*)\\.json$")
# HISTORY_PATTERN = re.compile("([0-9]*)\\.json$")
ATTACHMENT_PATTERN = re.compile(r"attachments\.*\w*\.(\w+)\/meta\.(\w*)\.json$")
FOLDER_PATTERN = re.compile("\\/(\\w*)\\/.dm\\/meta.folder.json$")
SPACES_PATTERN = re.compile("\\/(\\w*)\\/.dm\\/meta.space.json$")
